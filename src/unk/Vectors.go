...
// returns a vec 2d with coordinates ranging from 0 to +maxCoordinate
...
// returns a vec 2d with coordinates ranging from -maxCoordinate to +maxCoordinate
...
// sets a coordiante to a random value between -maxCoordinate and +maxCoordinate
func (vec *Vector2D) SetCoordinateToRandom(coor int, maxCoordinate float64) {
	if coor > 1 {
		return
	}
	sign := 1.0
	...
	vec[coor] = sign * rand.Float64() * maxCoordinate
